...
BaseCommand::BaseCommand(std::shared_ptr<libusb_device_handle *> device)
    : device_(device) {}
...
bool BaseCommand::IsMessageValid(
    const std::vector<unsigned char> &message) const {
  if (memcmp(this->valid_message_back_, message.data(),
             this->valid_message_back_size_) != 0) {
...
std::shared_ptr<libusb_device_handle *> BaseCommand::GetDevice() const {
  return device_;
}
