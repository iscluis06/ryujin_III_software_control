...
std::shared_ptr<testing::NiceMock<LibUsbWrapperMock>> TurnOffCommandTest::mock;
std::vector<unsigned char> TurnOffCommandTest::default_array;

TEST_F(TurnOffCommandTest, ExecuteSuccess) {
    EXPECT_CALL(*(mock.get()), FillArray).WillOnce(testing::Return(default_array));
    TurnOffCommand turn_off_command(mock);
