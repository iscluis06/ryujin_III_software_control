#include "commands/hardware_monitor_line_command.h"
#include <gtest/gtest.h>
#include "led_line/ryujin_liquid_temp_line.h"
#include "libusb_wrapper_mock.h"
#include "stores/hardware_status_store_mock.h"

class HardwareMonitorLineCommandTest : public testing::Test {
protected:
    static void SetUpTestSuite() {
        wrapper_mock = std::make_shared<LibUsbWrapperMock>();
        hardware_status_store_mock = std::make_shared<testing::NiceMock<HardwareStatusStoreMock>>();
        default_array = std::vector<unsigned char>(65, 0);
    }
    static void TearDownTestSuite() {
        wrapper_mock.reset();
        hardware_status_store_mock.reset();
    }
    static std::shared_ptr<LibUsbWrapperMock> wrapper_mock;
    static std::shared_ptr<testing::NiceMock<HardwareStatusStoreMock>> hardware_status_store_mock;
    static std::vector<unsigned char> default_array;
};

std::shared_ptr<LibUsbWrapperMock> HardwareMonitorLineCommandTest::wrapper_mock;
std::shared_ptr<testing::NiceMock<HardwareStatusStoreMock>> HardwareMonitorLineCommandTest::hardware_status_store_mock;
std::vector<unsigned char> HardwareMonitorLineCommandTest::default_array;

TEST_F(HardwareMonitorLineCommandTest, ExecuteSuccess) {
    EXPECT_CALL(*(wrapper_mock.get()), FillArray).WillRepeatedly(testing::Return(default_array));
    EXPECT_CALL(*(wrapper_mock.get()), SendInterrupt).WillRepeatedly(testing::Return(true));
    std::shared_ptr<RyujinLiquidTempLine> ryujin_liquid_temp_line =
            std::make_shared<RyujinLiquidTempLine>(hardware_status_store_mock);
    HardwareMonitorLineCommand hardware_monitor_line_command(wrapper_mock, ryujin_liquid_temp_line, 0);
    EXPECT_EQ(hardware_monitor_line_command.Execute(), true);
}

TEST_F(HardwareMonitorLineCommandTest, ExecuteFail) {
    EXPECT_CALL(*(wrapper_mock.get()), FillArray).WillRepeatedly(testing::Return(default_array));
    EXPECT_CALL(*(wrapper_mock.get()), SendInterrupt).WillRepeatedly(testing::Return(false));
    std::shared_ptr<RyujinLiquidTempLine> ryujin_liquid_temp_line =
            std::make_shared<RyujinLiquidTempLine>(hardware_status_store_mock);
    HardwareMonitorLineCommand hardware_monitor_line_command(wrapper_mock, ryujin_liquid_temp_line, 0);
    EXPECT_EQ(hardware_monitor_line_command.Execute(), false);
}
